...
- CAD の **向きを確定した図面を別名で登録**できる（後述の「図面の向き設定」UI）。CAD ソフトを持たない利用者のために、MAP 編集者があらかじめ基準の向きを直しておける
- 変換 API は別リポジトリ [growi-cad-convert-api](https://github.com/kawakin26/growi-cad-convert-api) を参照

### 図面の向き設定（CAD 登録・任意）
- ストック用ページ（既定 `/media-library`）で **フローティングボタン「🧭 図面の向き設定」** を表示（CAD 変換 API 設定時のみ）
- **「図面を登録」タブ**: ページ内の CAD を一覧（**ファイル名で絞り込み**、登録済みは **色分け＋「済」バッジ**）。選んで `0/90/180/270°` の向きをプレビューで確認し、**別名で登録**（登録時に SVG へ向きを焼き込む）。同じ図面を角度違いで何個でも登録できる
- **「登録済み一覧」タブ**: 登録済みの図面を一覧・確認（閲覧のみ）。各図面の **「別角度で再登録」** から、その元 CAD を引き継いで別名登録できる
- 登録した図面は、記法の `file` に **登録名** を指定するだけで、その向きで表示される
...
### 図面の向きを登録する（MAP 編集者向け）

CAD をアップロードした向きが正しくない場合、MAP 編集者が **向きを確定した図面を別名で登録**できます（CAD ソフトが不要）。

1. ストック用ページ（既定 `/media-library`）を開く
2. 右下の **「🧭 図面の向き設定」** ボタンをクリック
3. **「図面を登録」タブ**で CAD を選ぶ（ファイル名で絞り込める。登録済みは緑色＋「済」バッジで区別）
4. `0/90/180/270°` のボタンで向きを選び、プレビューで確認
5. **登録名**（記法の `file` に使う名前。元名と重複しない別名を推奨）を入力して **「この向きで登録」**
6. 以後、記法で `file="<登録名>"` と書くと、その向きで表示される

- 登録すると、その角度で **焼き込んだ SVG** が API 側に保存され、元 CAD から切り離された独立した図面として振る舞います（元 CAD を後で消しても表示され続けます）。
- **向きの変更はできません**。変えたい場合や CAD を修正したときは、**別名で再登録**してください（同じ元 CAD から角度違いを何個でも登録できます）。既存の登録は **「登録済み一覧」タブ**で確認でき、各図面の「別角度で再登録」から追加登録できます。
- **削除は UI から行えません**（既存ページを壊さないための措置。将来のサーバー管理ツールで対応予定）。
- 記法の `rotate`（表示時の CSS 回転）は、登録で焼き込んだ向きの **上にさらに上乗せ**されます。

> [!NOTE]
> このボタンは、CAD 変換 API（`cadConvertApi`）が設定され、かつ現在ページがストック用ページ（`defaultSrc`）のときだけ表示されます。ページの閲覧・編集権限を、ストック用ページ側で MAP 編集グループに限定しておくと、登録操作を編集者に限定できます。
...
- エントリはリポジトリ直下の `client-entry.tsx`（Vite の manifest キーを GROWI の期待に合わせるため直下に配置）。実装は `src/` に分割
  ...
  - `src/register.ts`: 図面の向き設定機能（ストックページのフローティングボタン → CAD を向き指定で別名登録・削除）。`activateRegister`
...
│   └── register.ts        # 図面の向き設定（CAD 登録・削除 UI）
├── dist/                  # ビルド成果物（コミット対象）
│   ├── assets/
│   │   └── client-entry-*.js
│   └── .vite/
