...
- **新規登録タブ**: 未登録の CAD を選び、`0/90/180/270°` の向きをプレビューで確認して **別名で登録**（登録時に SVG へ向きを焼き込む）
- **登録済み（削除）タブ**: 登録済みの図面を一覧・**削除**（向きの変更はできず、変えたい場合は別名で再登録する運用）
...
- **向きの変更はできません**。変えたい場合は別の登録名で再登録してください（**登録済み（削除）タブ**では削除のみ行えます）。
