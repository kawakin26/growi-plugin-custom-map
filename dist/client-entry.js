...
const getDefaultStockPage = () => {
    const cfg = window.GROWI_CUSTOM_MAP_CONFIG;
...
const apiv3Get = async (endpoint, params) => {
    ...
    if (!res.ok)
        throw new Error(`apiv3 GET failed: ${url} (${res.status})`);
    return res.json();
};
// ページパスから、そのページ ID を取得する
const getPageIdByPath = async (pagePath) => {
    try {
...
const getAttachmentsForPage = (pagePath) => {
...
const getCurrentPageAttachments = () => {
    try {
        return window.GROWI_CONTEXT?.page?.attachments || [];
    }
    catch {
        return [];
    }
};
const findAttachmentUrl = (attachments, fileName) => {
...
// 見つからなければ null。
const resolveAttachmentUrl = async (fileName, candidatePages) => {
    if (!fileName)
        return null;
    // まず現在ページの同期コンテキストを試す(候補に現在ページが含まれる場合の高速パス)
    const currentPath = window.GROWI_CONTEXT?.page?.path;
...
const initMapPopups = () => {
    ...
    elements.forEach((el) => {
        ...
        if (!mapData.currentPagePath) {
            mapData.currentPagePath = window.GROWI_CONTEXT?.page?.path || '';
        }
...
// ==========================================
const getMapCandidatePages = (mapData) => {
    const pages = [];
    if (mapData.src)
        pages.push(mapData.src);
...
// ==========================================
const getPhotoCandidatePages = (mapData, marker) => {
    const pages = [];
    if (marker.photoSrc)
        pages.push(marker.photoSrc);
    if (mapData.currentPagePath)
        pages.push(mapData.currentPagePath);
...
// モーダルの生成と各種インタラクション
// ==========================================
const openMapModal = async (mapData) => {
    // 既存モーダルの除去
    ...
    viewport.addEventListener('pointerdown', (e) => {
        // マーカー等の操作はパンにしない
        if (e.target.closest('[data-map-marker]'))
            return;
        dragging = true;
        startX = e.clientX;
        startY = e.clientY;
...
// ==========================================
const createMarker = (stage, marker, mapData) => {
    ...
    const attachInteractions = (elForActions) => {
        ...
        elForActions.addEventListener('pointerdown', (e) => {
            // パン処理へ伝播させない
            e.stopPropagation();
            longPressed = false;
        ...
        elForActions.addEventListener('pointermove', (e) => {
        ...
        elForActions.addEventListener('pointerup', (e) => {
            e.stopPropagation();
            if (longPressTimer) {
                window.clearTimeout(longPressTimer);
                longPressTimer = undefined;
...
// ==========================================
const openPhotoPopup = (photoUrl, caption) => {
...
const buildMapData = (node) => {
    const attributes = node.attributes || {};
    const markers = [];
    // 子ノードのうち listItem を走査してマーカー化
    (0, unist_util_visit_1.visit)(node, 'listItem', (listItem) => {
...
const activate = () => {
    if (typeof growiFacade === 'undefined'
        || growiFacade == null
    ...
    optionsGenerators.customGenerateViewOptions = (...args) => {
        ...
        options.remarkPlugins.push(() => (tree) => {
...
const pluginDefinition = {
    ...
    activatePlugin: exports.activate,
    deactivatePlugin: exports.deactivate,
};
if (typeof window !== 'undefined') {
